/*!
\file   traum.cpp
\brief  Исходный файл функционала истории "Traum" с выбором ответа (интерфейс истории с выбором ответа)
\author Кожан Дарья
\date   31.12.2022
*/
    ...
    ui(new Ui::Traum)
{
    ...
    ui->setupUi(this);
    showFullScreen();
    QFile file("../../Data/Traum/" + QString::number(count) + ".txt"); //путь к нужному файлу
    ...
    QFile file1("../../Data/Traum/" + QString::number(count) + ".txt"); //путь к нужному файлу
    if(!file1.open(QIODevice::ReadOnly))
        return;
    QString var1 = QByteArray(file1.readAll());
    ui->pushButton->setText(var1); //текст кнопки
    count++;

    QFile file2("../../Data/Traum/" + QString::number(count) + ".txt"); //путь к нужному файлу
    if(!file2.open(QIODevice::ReadOnly))
        return;
    QString var2 = QByteArray(file2.readAll());
    ui->pushButton_3->setText(var2); //текст кнопки
...
void Traum::on_action_triggered()
{
...
void Traum::on_action_5_triggered()
{
